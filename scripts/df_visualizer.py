...
def plotly_plot_pie(df, column, limit=None, title=None):
    ...
    # If a limit is specified, group values below this limit under a single 'Other' category
    if limit:
        a.loc[a['count'] < limit, column] = f'Other {column}s'
        a = a.groupby(column).sum().reset_index()  # Re-group to combine 'Other' entries

    # Set the title if none is provided
    if title is None:
